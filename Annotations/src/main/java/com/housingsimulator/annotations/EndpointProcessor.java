...
public class EndpointProcessor extends AbstractProcessor {
        ...
        //if(roundEnvironment.processingOver()) {
            try {
                ...
                JavaFileObject builderFile = processingEnv.getFiler()
                        .createSourceFile("com.housingsimulator.controller.MainController");
    ...
    private String processClasses(RoundEnvironment roundEnvironment) throws IllegalAnnotationException {
        ...
        for(Element annotatedElement : roundEnvironment.getElementsAnnotatedWith(API.class)) {
            if(annotatedElement.getKind() != ElementKind.CLASS) {
                error(annotatedElement, "@Controller must be applied to classes");
                throw new IllegalAnnotationException("@Controller must be applied to classes");
            }
            ...
            builder.append(String.format("\t\tcontrollers.put(\"%s\", new %s(this.getModel()));\n", name, name));
    ...
    private String methodCall(Element element) {
        ...
        for(Element e : ((ExecutableElement)element).getParameters()) {
            ...
            if(i != 0)
                callBuilder.append(",");
            callBuilder.append(String.format("new %s(m.group(%d))", type, i + 1));
